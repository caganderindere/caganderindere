<div align="center">

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&height=260&color=0:0F172A,50:2563EB,100:38BDF8&text=ÇAĞAN%20DERİNDERE&fontSize=52&fontColor=ffffff&animation=fadeIn"/>

# 👋 Merhaba, Ben Çağan Derindere

<img src="https://readme-typing-svg.demolab.com?font=JetBrains+Mono&weight=700&size=28&duration=3000&pause=900&color=38BDF8&center=true&vCenter=true&width=900&lines=Full+Stack+.NET+Developer;ASP.NET+Core+Developer;Backend+Developer;Shopify+Developer;E-Commerce+Solutions;Always+Learning+%F0%9F%9A%80" />

### 💻 Full Stack .NET Developer • ASP.NET Core • Shopify Developer

Modern ASP.NET Core uygulamaları geliştiriyor, e-ticaret altyapıları oluşturuyor ve ölçeklenebilir web çözümleri üretiyorum.

🚀 **Hedefim; performanslı, sürdürülebilir ve kullanıcı odaklı yazılımlar geliştirmek.**

<br>

<a href="mailto:programmercagan@gmail.com">
<img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<a href="https://www.linkedin.com/in/cagandrd/">
<img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin"/>
</a>

<a href="https://github.com/caganderindere">
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github"/>
</a>

<img src="https://komarev.com/ghpvc/?username=caganderindere&style=for-the-badge&color=3B82F6"/>

</div>

---

# 👨‍💻 Hakkımda

```csharp
public class CaganDerindere
{
    public string Role =>
        "Full Stack .NET Developer";

    public string Education =>
        "Management Information Systems";

    public string[] Focus =
    {
        "ASP.NET Core",
        "Backend Development",
        "REST API",
        "Entity Framework Core",
        "Software Architecture",
        "Shopify Development",
        "E-Commerce Solutions",
        "SQL Server"
    };

    public string Motto =>
        "Build. Learn. Improve. Repeat.";
}
```

---

# 🎯 Odak Noktam

- 🚀 ASP.NET Core ile modern web uygulamaları geliştirmek
- ⚙️ RESTful API ve backend servisleri oluşturmak
- 🛒 E-ticaret çözümleri geliştirmek
- 📦 E-ticaret süreçlerini optimize eden sistemler tasarlamak
- 🏗️ Temiz, sürdürülebilir ve ölçeklenebilir yazılım mimarileri oluşturmak

---

# 🛠️ Teknolojiler

### Backend

`C#` • `ASP.NET Core` • `Entity Framework Core` • `REST API` • `JWT` • `SignalR`

### Frontend

`HTML` • `CSS` • `JavaScript` • `Bootstrap`

### Database

`SQL Server` • `PostgreSQL` • `MongoDB` • `Redis`

### E-Commerce

`Shopify` • `WooCommerce` • `SEO` • `Performance Optimization`

### Tools

`Git` • `GitHub` • `Docker` • `Visual Studio` • `VS Code`

---

# 📊 GitHub İstatistikleri

<p align="center">

<img height="170" src="https://github-readme-stats.vercel.app/api?username=caganderindere&show_icons=true&theme=tokyonight&hide_border=true"/>

<img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=caganderindere&layout=compact&theme=tokyonight&hide_border=true"/>

</p>

<p align="center">

<img src="https://streak-stats.demolab.com?user=caganderindere&theme=tokyonight&hide_border=true"/>

</p>

---

# 📈 Development Activity

<p align="center">

<img src="https://github-readme-activity-graph.vercel.app/graph?username=caganderindere&theme=tokyo-night&hide_border=true"/>

</p>

---

# 📫 İletişim

<p align="center">

<a href="mailto:programmercagan@gmail.com">
<img src="https://img.shields.io/badge/Gmail-EA4335?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>

<a href="https://www.linkedin.com/in/cagandrd/">
<img src="https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin"/>
</a>

<a href="https://github.com/caganderindere">
<img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github"/>
</a>

</p>

---

> ## 💙 "Çocukken bilgisayarın karşısında başlayan merakım, bugün ASP.NET Core tabanlı uygulamalar geliştirerek ve e-ticaret projelerine değer katarak yolculuğuna devam ediyor."

<div align="center">

### ⭐ Build. Learn. Improve. Repeat.

<img width="100%" src="https://capsule-render.vercel.app/api?type=waving&section=footer&height=140&color=0:0F172A,50:2563EB,100:38BDF8"/>

</div>
